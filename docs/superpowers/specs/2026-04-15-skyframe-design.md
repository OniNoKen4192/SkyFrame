...
- **Location:** hardcoded lat/lon `{LAT}, {LON}` (ZIP {ZIP}, {CITY, STATE}).
...
- **User-Agent header (required by NWS):** `SkyFrame/0.1 (your-email@example.com)`.
...
|---|---|---|
| `GET /points/{lat},{lon}` | One-time metadata resolution (grid, radar, zone, astronomicalData) | Forever (resolved at server startup, never re-fetched) |
...
| `GET /alerts/active?point={lat},{lon}` | Weather alerts (deferred to v1.1, not v1) | n/a |

**All requests include:** `User-Agent: SkyFrame/0.1 (your-email@example.com)` and `Accept: application/geo+json`.
