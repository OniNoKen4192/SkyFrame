...
| State | Surrounding text (`■ SKYFRAME · {CITY} {ZIP} ·`) | Link text |
|---|---|---|
...
2. **Online baseline:** Footer shows cyan pulsing dot + `LINK.<stationId>` + `NEXT` ~90s out. TopBar location line shows `■ SKYFRAME · {CITY} {ZIP} · LINK.<stationId>` in default cyan.
