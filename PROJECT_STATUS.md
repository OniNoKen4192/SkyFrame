...
- AlertBanner silencing is now explicit: a dedicated mute glyph (🔇, forced monochrome via variation selector) appears on the banner's right side whenever any visible alert has an active repeating sound. The old click-anywhere `onClick` on the banner root is gone — opening the detail modal or expanding the multi-alert list no longer silences. Glyph shows only when a looping alert is un-acknowledged; disappears after click or when the alert drops off the feed. Styled to match the sibling `▾` toggle and `×` dismiss glyphs. Powered by a new pure `anyAlertLooping(alerts, acknowledged)` predicate in `client/sound/alert-sounds.ts` with 5 unit tests.
...
- **NWS fetch timeout** — `fetchNws` now wraps `fetch()` in a per-attempt `AbortController` with a configurable timeout (`CONFIG.nws.timeoutMs`, default 10000ms). `AbortError` is classified as the previously-declared-but-unwired `NwsErrorCode 'timeout'`. Retried once same as network errors. Previously a hung NWS connection would block `/api/weather` indefinitely, stalling the client's 90s poll cycle. Two new tests in [server/nws/client.test.ts](server/nws/client.test.ts) verify the signal is passed and that double-timeout produces an `NwsError('timeout')`.
- **Setup completion refreshes client config** — `handleSetupComplete` in [client/App.tsx](client/App.tsx) now calls `fetchConfig()` after a successful first-run setup. Previously the dashboard was rendered with `timezone`, `stationOverride`, `primaryStationId`, and `fallbackStationId` still null until the next reload — the station override popover had nothing to anchor on and timestamps fell back to the browser's local TZ instead of the NWS-derived one. Mirrors the pattern already used by `handleOpenSettings`.
- **Vitest test-isolation** — added explicit `test.include` (`{client,server,shared}/**/*.test.ts`) and extended `exclude` in [vite.config.ts](vite.config.ts). Vitest's default `**/node_modules/**` exclude doesn't match sibling backup folders like `node_modules.bak/`, which would otherwise cause `npm test` to pick up vendored dependency tests and fail with unrelated errors.
- All four fixes originated in the v1.2.6 senior-dev review pass; the fallback-validation and fetch-timeout findings were caught by a second-opinion sweep that specifically traced defensive-check call sites and declared-but-unwired error codes.
